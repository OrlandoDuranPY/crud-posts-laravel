...
<div>
    ...
    <textarea name="content" id="content"
        class="resize-none rounded-md border {{ $errors->has('content') ? 'border-red-800' : 'border-gray-500' }}">{{ old('content', $post->content) }}</textarea>
</div>
{{-- Description --}}
<div>
    <label for="description">Descripcion:</label>
    <textarea name="description" id="description"
        class="resize-none rounded-md border {{ $errors->has('description') ? 'border-red-800' : 'border-gray-500' }}">{{ old('description', $post->description) }}</textarea>
</div>
{{-- Image --}}
<div>
